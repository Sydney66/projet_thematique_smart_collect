# -*- coding: utf-8 -*-
"""
Created on Sun Nov 16 18:34:39 2025
...
def compute_area_green(angles, heights):
    x = heights * np.cos(angles)
    y = heights * np.sin(angles)
    area = 0.5 * np.abs(np.dot(x, np.roll(y, -1)) - np.dot(y, np.roll(x, -1)))
    return area

def compute_volume(angles, heights):
    avg_height = np.mean(heights)
    r = np.mean(heights)
    volume = np.pi * r**2 * avg_height
    return avg_height, volume

def compute_fill_percentage(volume_cm3, bin_volume_cm3=100000):
    return (volume_cm3 / bin_volume_cm3) * 100
...
def create_final_plot(angles, heights, area, avg_height, volume, fill_pct, outdir):
    ...
    plt.text(0.05, 0.95, f"Area: {area:.2f} cm²\nAvg Height: {avg_height:.2f} cm\nVolume: {volume:.2f} cm³\nFill: {fill_pct:.1f}%",
    ...
    np.savetxt(csv_path, np.column_stack((angles, heights)), delimiter=',', header='angle_rad,height_cm', comments='')
